...
typedef struct {
	...
	odpx_comp_session_param_t session; /**< Prefilled comp session params */
} comp_alg_config_t;
...
static int
create_session_from_config(odpx_comp_session_t *session,
			   comp_alg_config_t *config,
			   comp_args_t *cargs)
{
			   ...
			   odpx_comp_op_result_t  *comp_result)
{
	...
	if (cargs->poll) {
		...
		if (rc < 0) {
			app_err("failed odpx_comp_compress_enq: rc = %d\n",
				rc);
			return -1;
		}
			     ...
			     odpx_comp_op_result_t  *comp_result)
{
	...
	if (cargs->poll) {
		...
		if (rc < 0) {
			app_err("failed odpx_comp_decomp_enq: rc = %d\n",
				rc);
			return -1;
		}
...
static void parse_args(int argc, char *argv[], comp_args_t *cargs)
{
	...
	odpx_comp_alg_capability_t  capa;
