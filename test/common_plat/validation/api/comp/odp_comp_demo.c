...
static int process(comp_args_t *, odpx_comp_op_param_t *);
...
static int consume_data(odpx_comp_op_result_t *result, FILE *out)
{
	uint32_t len = 0;
	uint32_t offset;
	uint8_t *data;
...
#if 0
static void test_sha(comp_args_t *pcargs)
{
	int ret;
	odp_pool_t                 pkt_pool;
	odp_packet_t               pkt = ODP_PACKET_INVALID;
	odpx_comp_session_t         session = ODP_COMP_SESSION_INVALID;
	...
	params.hash_alg = ODP_COMP_HASH_ALG_SHA1;
	digest = sha1_digest;
	digest_len = sizeof(sha1_digest);

	if (strcmp(pcargs->alg, "sha256") == 0) {
		params.hash_alg = ODP_COMP_HASH_ALG_SHA256;
	...
	ret = odpx_comp_session_create(&params, &session, &status);
	if (ret && status != ODP_COMP_SES_CREATE_ERR_NONE) {
		app_err("Session creation failed\n");
		return;
	}

	pkt = odp_packet_alloc(pkt_pool, sizeof(str) - 1);

	if (ODP_PACKET_INVALID == pkt) {
		app_err("Failed to allocate packet\n");
		odpx_comp_session_destroy(session);
		return;
	}

	odp_packet_copy_from_mem(pkt, 0, sizeof(str) - 1,  str);

	op_params.session = session;
	op_params.input.packet.packet  = pkt;
	op_params.input.packet.data_range.offset  = 0;
	op_params.input.packet.data_range.length = odp_packet_len(pkt);
	op_params.output.packet.packet = odp_packet_alloc(pkt_pool, digest_len);
	op_params.output.packet.data_range.offset  = 0;
	op_params.output.packet.data_range.length =
				odp_packet_len(op_params.output.packet.packet);
	op_params.last = 1;

	app_dbg("len %d\n", op_params.output.packet.data_range.length);

	ret = odpx_comp_operation(&op_params, &result);
	if (ret || (result.err != ODP_COMP_ERR_NONE)) {
		app_err("Operation Failed.ret %d, result.err %d\n", ret,
			result.err);
	}

	data = odp_packet_data(result.output.packet.packet);
	len = result.output.packet.data_range.length;
	...
	if (memcmp(digest, data, len))
		app_err("Digest didn't match\n");

	/* free up pkt used by operation */
	odp_packet_free(result.output.packet.packet);
	odp_packet_free(op_params.input.packet.packet);
	odpx_comp_session_destroy(session);
}
#endif

static int process(comp_args_t *pcargs, odpx_comp_op_param_t *op_params)
{
	int ret;
	odp_event_t ev;
	odpx_comp_op_result_t result;
	...
	do {
		...
		if (pcargs->async) {
			ret = (pcargs->compress) ?
				odpx_comp_compress_enq(op_params) :
				odpx_comp_decomp_enq(op_params);
			ev = odp_queue_deq(pcargs->compl_queue);
			odp_packet_t compl_ev =
			odpx_comp_packet_from_event(ev);
			odpx_comp_result(compl_ev,
					 &result);
		} else {
			ret = (pcargs->compress) ?
				odpx_comp_compress(op_params, &result) :
				odpx_comp_decomp(op_params, &result);
		}
...
static int init(comp_args_t *pcargs, odpx_comp_op_param_t *op_param)
{
	int                       ret = 0;
	odpx_comp_session_t         session = ODP_COMP_SESSION_INVALID;
	odpx_comp_session_param_t   params;
	...
	memset(&params, 0, sizeof(odpx_comp_session_param_t));
...
static int term(comp_args_t *pcargs, odpx_comp_op_param_t *param)
{
	odpx_comp_session_destroy(param->session);
	fclose(pcargs->in);
	fclose(pcargs->out);
	return 0;
}
...
static void test_comp(comp_args_t *pcargs)
{
	...
	int                         err = 0;
	int32_t                     iteration;
	odpx_comp_data_t *in = &op_params.input;
	odpx_comp_data_t *out = &op_params.output;
